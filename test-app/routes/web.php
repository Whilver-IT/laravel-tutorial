...
Route::prefix('goods')->name('goods.')->controller(GoodsController::class)->group(function () {
    ...
    // matchを使用するとmethodの指定ができ、下記はGETとPOST両方GoodsControllerのsearchメソッドを呼び出す
    Route::match(['get', 'post'], '/search', 'search')->name('search');

    // 「{」、「}」で囲まれた名称(以下の場合、{id})をdetailメソッドの引数で受け取れる
    // 受け取った側(detailメソッド)で変数の検証を行ってもよいが、whereの正規表現で以下のように指定できる
    // 以下はアルファベットと数字の例
    Route::get('/detail/{id}', 'detail')->where('id', 'A-z0-9')->name('detail');
    ...
    Route::prefix('input')->group(function () {
        ...
        Route::match(['get', 'post'], '/{id?}', 'input')->name('input');
